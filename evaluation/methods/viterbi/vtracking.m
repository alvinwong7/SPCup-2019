%% vtracking.m -- viterbi-based flight tracking
function DOA = vtracking(wavforms,fs_wav,params, sourceData, fileNum, motor_speed)
%% SETTINGS
data = char(params{1});
...
if ~split
    ...
    for t = 1:T
        DOA = [DOA; sources(t,argmax(t),1) sources(t,argmax(t),2)];
        if plotting
    ...
    for i = 1:ceil(T/splitFrames)
        s = 1+(i-1)*(splitFrames+1);
        e = s+splitFrames;
        if e > T
           e = T;
        end
        [argmax, valmax] = viterbi(e-s+1, sources(s:e,:,:), emission(s:e,:));
        for t = 1:length(argmax)
            DOA = [DOA; sources(t,argmax(t),1) sources(t,argmax(t),2)];
            if plotting
                openfig("angularSpectrum" + s+t-1 + ".fig");
                hold on
                scatter3(sources(t,argmax(t),1),sources(t,argmax(t),2), 1, 'o','lineWidth',2);
                hold off
                savefig("angularSpectrum" + t + ".fig");
