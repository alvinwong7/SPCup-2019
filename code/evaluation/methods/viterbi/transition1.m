...
function p = transition1(t1,p1,t2,p2,k)
    if isfile(['min_factor_trans1_' num2str(k) '.mat'])
        load(['min_factor_trans1_' num2str(k) '.mat']);
    else
        factor = 0;
        ...
        save(['min_factor_trans1_' num2str(k) '.mat'],'min','factor');
    end
